...
public class MySQL5FriendListDAO extends FriendListDAO {
	...
	@Override
	public FriendList load(final PlayerCommonData playerCd) {
		final List<Friend> friends = new ArrayList<>();
		Connection con = null;
		try {
			con = DatabaseFactory.getConnection();
			PreparedStatement stmt = con.prepareStatement(LOAD_QUERY);
			stmt.setInt(1, playerCd.getPlayerObjId());
			...
			log.error("Could not restore QuestStateList data for player: " + playerCd.getPlayerObjId() + " from DB: " + e.getMessage(), e);
		}
		finally {
			DatabaseFactory.close(con);
		}

		return new FriendList(playerCd, friends);
	}
	 ...
	 */
	@Override
	public boolean addFriends(final Player player, final int friendId) {
