...
public class IlluminaryObeliskInstance extends GeneralInstanceHandler {
	...
	public void onDie(Npc npc) {
		...
		switch (npc.getObjectTemplate().getTemplateId()) {
			...
			case 233740: // Test Weapon Dynatoum.
				...
				spawn(730905, 255.5496f, 270.494967f, 455.12012f, (byte) 90); // Illuminary Obelisk Exit.
				break;
		}
	}
	...
	public void onEnterZone(Player player, ZoneInstance zone) {
		if (zone.getAreaTemplate().getZoneName() == ZoneName.get("SHIELD_GENERATION_HUB_301230000")) {
			if (!isStop && !player.isProtectionActive()) {
				isStop = true;
				sendMsg(1402143);
	...
	public void spawncheckE() {
		if (!isEnd) {
			...
			}, 120 * 1000); // 2 Dakikada bir çıkan slotlar
		}
	}
	...
	private void spawnShieldControlRoomTeleporter() {
		instance.doOnAllPlayers(new Visitor<Player>() {
			...
			public void visit(Player player) {
				if (player.isOnline()) {
					PacketSendUtility.sendPacket(player, new SM_QUEST_ACTION(0, 0)); // 6 Minutes.
				}
			}
		});
		sendMsg(1402202);
