...
public class SM_CHARACTER_LIST extends PlayerInfo {
     ...
     */
    public SM_CHARACTER_LIST(int accountId) {
    ...
    protected void writeImpl(AionConnection con) {
        ...
        for (final PlayerAccountData playerData : account.getSortedAccountsList()) {
            ...
            writeB(new byte[40]); // unk 4.5.0.18
            if (cbi != null && cbi.getEnd() > ((System.currentTimeMillis() / 1000) + 7200)) {
                ...
                writeB(new byte[52]); // unk 4.5.0.18
                writeD(playerData.getDeletionTimeInSeconds()); // 4.5.0.18
                writeH(player.getCommonData().getPlayerSettings().getDisplay());//display helmet 0 show, 5 dont show
                writeH(player.getCommonData().getPlayerSettings().getDeny());
                ...
                writeB(new byte[34]); // unk 4.5.0.18
            }
        }
    }
