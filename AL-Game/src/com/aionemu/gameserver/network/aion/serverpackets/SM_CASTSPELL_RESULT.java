    ...
    protected void writeImpl(AionConnection con) {
        ...
        for (Effect effect : effects) {
            ...
            if (effected != null) {
                ...
                writeC((int) (100f * effected.getLifeStats().getCurrentHp() / effected.getLifeStats().getMaxHp())); // target %hp
            ...
            if(effect.isMpAttack()) {
                ...
                writeC(1);
                writeD(effect.getReserved2());
                writeC(10);
                writeC(0);

            } else {
            	writeC(1); // Subeffect size
                writeC(effect.isFpHeal() ? 2 : effect.isMphealInstant() ? 1 : 0);
                if (effect.isDelayedDamage()) {
                    writeD(0);
                } else {
                    writeD(effect.isFpHeal() ? effect.getReserved2() : effect.getReserved1());
                }
                writeC(effect.getAttackStatus().getId());
